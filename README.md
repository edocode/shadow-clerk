...
A bar highlights yellow after 10 seconds of loud-but-flat audio (steady noise, no speech dynamics), and red after 30 seconds of exact silence — a live device always has some noise floor, so true zero means nothing is reaching it at all (e.g. a powered-off headset whose dongle still shows up as a device). It also gets a fallback outline when the configured device for that path is unavailable and the automatic one is in use, matching the fallback described above.
