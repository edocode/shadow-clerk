...
# 30秒→20秒に短縮 (2026-08): 実配置 (ReazonSpeech K2, fp32, cpu, main/interim で
# モデル共有 - commit 45333fe) を in-process で再現して実測。
# 単独コールの限界コスト(新規プロセスでロード直後に1回だけ transcribe): 10秒 +81MB,
# 30秒 +301MB。ただしセッション定常状態 (main+interim 共有・20segment の混在長
# ストリームを流した後、3 seed) では cap=30秒が約1266-1276MB, cap=20秒が
# 約1153-1157MBに収束し、差は約115MB — 一度大きいセグメントでヒープが伸びると
# 後続の同等以下のセグメントでは縮まず高止まりするため、単独コールの差(約220MB)
# より小さくなる。20秒は強制分割のトレードオフを払ってこの高止まりの天井を下げる。
