...
- **AI Console cursor jump on Windows (ConPTY)**: On Windows only (reproduced via pywinpty; not seen over a POSIX pty on Linux), the `claude` child process occasionally jumps its cursor thousands of rows down right after launch before drawing its first frame. `_consoleRowEl()` fills every row in between with blank `.cr` elements to keep row numbers contiguous, which used to show up as a wall of blank space above the prompt. Fixed client-side in `_daemon_dashboard_js_console.py` by tracking `_consoleFirstRow` (the first row that ever got real content) and hiding everything above it, mirroring how `_trimConsoleTail()` already hides everything below the last real content.
