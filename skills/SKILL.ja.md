...
4. 音声コマンド監視用のバックグラウンド subagent を Task ツール（`run_in_background: true`）で起動する。プロンプトには以下を必ず含めること:
   ...
   - subagent は以下のループを実行する:
     ...
     - `translate_regenerate` で始まるコマンドを検出したら（例: `translate_regenerate 202603091914`）:
       - `clerk-util write .clerk_command ""` でクリア
       - `clerk-util read-config` で `translate_language` を取得
       - コマンドから日時部分を取得（例: `202603091914`）
       - 対象 transcript ファイルは `transcript-<日時部分>.txt`
       - オフセットをリセット: `clerk-util write transcript-<日時部分>.txt.translate_offset 0`
       - ファイル全体を一括翻訳する（過去ファイルは増えないのでループせず、翻訳完了後にコマンドポーリングに戻る）
...
5. ループ開始:
   ...
   b. 新しい行があれば:
      - **`translation_provider: claude`（デフォルト）の場合** — 以下を Claude 自身がインラインで実行する:
        - `clerk-util read glossary.txt` で用語集を読む（ファイルがなければスキップ）
        - ヘッダーから翻訳先言語（`<lang>`）の列を特定し、用語集の対応する訳語を把握する
        - 用語集の `reading` 列を使って、音声認識の漢字誤変換を特定する（例: 習性→reading「しゅうせい」→ 用語集の「修正」にマッチ）
        - **同言語（例: ja→ja）の場合**: 翻訳ではなく**音声認識の誤変換修正**が主目的。各単語の読みを考え、用語集の reading マッピングや文脈から正しい漢字・表記に修正する
        - **異言語の場合**: 音声認識の誤認識を修正してから翻訳する
        - `--- 会議開始 ---` や `--- 会議終了 ---` 等のマーカー行は修正せずそのまま出力する
        - タイムスタンプ `[YYYY-MM-DD HH:MM:SS]` とスピーカーラベル `[自分]` `[相手]` 等はそのまま保持し、テキスト部分のみ翻訳/修正する
          - 例（ja→en）: `[2026-02-27 14:30:00] [自分] プロジェクトのタイムラインについて話しましょう。` → `[2026-02-27 14:30:00] [自分] Let's discuss the project timeline.`
          - 例（ja→ja）: `[2026-02-27 14:30:00] [相手] その習性が原因かなと` → `[2026-02-27 14:30:00] [相手] その修正が原因かなと`（習性→修正: 同じ読み「しゅうせい」、文脈はバグ修正の話）
      ...
      - 翻訳/修正結果を `<transcriptのベース名>-<lang>.txt` に書き込む（出力ディレクトリは `clerk-util read-config` で `output_directory` を確認。null なら data ディレクトリ `{data_dir}` を使用）
        - 例: `transcript-20260227.txt` → `transcript-20260227-ja.txt`
        - 例: `transcript-202602271430.txt` → `transcript-202602271430-ja.txt`
        - **Write ツール**（clerk-util ではなく）で翻訳ファイルを直接書き込むこと。Write ツールにはフルパスを指定する（例: `{data_dir}/transcript-20260227-ja.txt`）
        - 最初のチャンク（offset が 0 だった場合）: Write ツールでファイルを作成/上書き
        - 以降のチャンク: Edit ツールで追記するか、再読み込みして Write ツールで全体を書き込む
      - 翻訳結果を stdout にも表示する（print）
      - `clerk-util write <transcript>.translate_offset <offset>` にバイトオフセットを更新して書き込む
...
### glossary.txt（用語集 + 読み置換)
- TSV 形式（タブ区切り）で用語集を定義する
- 1行目はヘッダー行。言語コード（`ja`, `en` 等)と、特殊列として `reading`（音声認識の読み)、`note`（補足)を含めることができる
- 2行目以降に各言語に対応する用語を記載する
- `#` で始まる行はコメント、空行は無視される
- **2通りの用途で使われる:**
  1. **機械置換**: `reading` 列の値が ASR 出力に現れたら対応する言語列の値に置換する（例: `エーアイ → AI`)。長さ降順で照合(長い reading を優先)し、部分マッチの誤置換を防ぐ。clerk-daemon が transcript 保存前に適用
  2. **LLM ヒント**: 翻訳・要約時にプロンプトに渡して訳語を統一する
