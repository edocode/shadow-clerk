...
「音量はあるのに変化がない」状態が10秒続くとバーが黄色（定常ノイズ）に、完全な無音（音量が正確にゼロ）が30秒続くと赤色（無音デバイス）になる。生きたデバイスには常にノイズフロアがあるため、厳密にゼロが続くのは音が届いていない証拠——例えば電源が切れた状態でもドングルだけはデバイスとして認識されるヘッドセットなど。指定デバイスが使えず自動デバイスで代替中の系統には、上記のフォールバックと同じ意味でリング状の枠が付く。
